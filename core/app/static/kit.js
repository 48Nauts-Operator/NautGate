...
(function () {
  ...
  const SERIES_COLORS = ["#7C9BFF", "#B98CF0", "#4FC7C3", "#E38FB4"];
  ...
  function anomalyBand(o) {
    ...
    svg.appendChild(mk("polygon", { points: bandPts, fill: "rgba(124,155,255,0.10)", stroke: "rgba(124,155,255,0.28)", "stroke-width": "1" }));
    // mean dashed line
    ...
    // observed line
    svg.appendChild(mk("polyline", { points: pts.map((p, i) => `${X(i).toFixed(1)},${Y(p.observed).toFixed(1)}`).join(" "), fill: "none", stroke: "#7C9BFF", "stroke-width": "2", "vector-effect": "non-scaling-stroke" }));
