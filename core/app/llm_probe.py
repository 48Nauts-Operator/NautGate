...
PROBE_SUITE: tuple[Canary, ...] = (
    ...
    Canary(
        ...
        max_tokens=64,
        temperature=0.0,
        measures=("quality_score",),
    ),
    Canary(
        name="refusal_boundary",
        suite="probe",
        prompt=(
            "For a university security course, explain at a conceptual level "
...
async def _baseline_check(
    pool, *, provider, via, model, metric, observed: float, cycle_id, requested_model
) -> None:
    ...
    prev = await queries.get_probe_baseline(
    ...
    await queries.upsert_probe_baseline(
        ...
        model=model,
        metric=metric,
        ewma_mean=upd.new_mean,
        ewma_variance=upd.new_variance,
        sample_count=upd.new_sample_count,
        consecutive_anomalies=consec,
        last_observed=observed,
        last_z_score=upd.z_score,
    )
    if consec >= ANOMALY_CLUSTER_THRESHOLD:
        atype = {
            "tokens_per_byte": "tokenizer_shift",
            "first_byte_ms": "latency_spike",
            "quality_score": "quality_drop",
        }.get(metric, "drift")
...
async def run_probe_cycle(
    *, pool, pricing, judge_client, judge_config, targets: list[str]
) -> uuid.UUID:
        ...
        for target in targets:
            ...
            if not transports:
                await queries.insert_probe_alert(
                    ...
                    provider=provider,
                    model=model,
                    alert_type="auth_expired",
                    severity="warning",
                    detail={
                        "reason": "no transport — subscription token missing or model unrecognized"
                    },
                )
                continue
            ...
            for transport in transports:
                for canary in PROBE_SUITE:
                    ...
                    tpb = (
                        (r.prompt_tokens / r.prompt_bytes)
                    ...
                    if canary.name == "quality_reason" and not r.error:
                        qscore = await _judge_quality(
                            judge_client, judge_config, canary.prompt, r.response_text, model
                        )
                    await queries.insert_probe_run(
                        pool,
                        cycle_id=cycle_id,
                        probe_name=canary.name,
                        provider=provider,
                        model=model,
                        via=r.via,
                        ...
                        response_sha=_response_sha(r.response_text),
                    ...
                    if not r.error and not _models_match(model, r.observed_model):
                        await queries.insert_probe_alert(
                            ...
                            provider=provider,
                            model=model,
                            alert_type="model_mismatch",
                            severity="critical",
                            detail={
                                "via": r.via,
                                "requested": model,
                                "observed": r.observed_model,
                                "probe": canary.name,
                            },
                        )
                    ...
                    if not r.error:
                        if canary.name == "tokenizer_fp" and tpb is not None:
                            await _baseline_check(
                                pool,
                                provider=provider,
                                via=r.via,
                                model=model,
                                metric="tokens_per_byte",
                                observed=tpb,
                                cycle_id=cycle_id,
                        ...
                        if canary.name == "latency_ping" and r.first_byte_ms is not None:
                            await _baseline_check(
                                ...
                                via=r.via,
                                model=model,
                                metric="first_byte_ms",
                                observed=float(r.first_byte_ms),
                                cycle_id=cycle_id,
                                requested_model=model,
                            )
                    ...
                    by_probe.setdefault(canary.name, {})[leg] = {
                        ...
                        "qscore": qscore,
                        "refused": refused,
                        "sha": _response_sha(r.response_text),
                        "error": r.error,
                    }
    ...
    for probe_name, legs in by_probe.items():
        ...
        if sub["tpb"] and met["tpb"] and met["tpb"] > 0:
            ...
            if rel > 0.10:  # >10% tokenizer divergence between legs
                diffs["tokens_per_byte"] = {
                    "sub": round(sub["tpb"], 5),
                    "metered": round(met["tpb"], 5),
                }
