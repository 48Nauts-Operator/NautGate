...
_PING = Canary(name="heartbeat", suite="heartbeat",
               prompt="Reply with: OK", max_tokens=1, temperature=0.0)
...
async def ping_once(client: httpx.AsyncClient, provider: str, model: str, pricing,
                    require_via: str | None = None) -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(15.0, connect=5.0), http2=False) as client:
                for label, provider, model, require_via in DEFAULT_TARGETS:
                    try:
                        res = await ping_once(client, provider, model, pricing, require_via=require_via)
                    except Exception as exc:
                        res = {"status": "down", "status_code": None, "latency_ms": None,
                               "via": None, "error": str(exc)}
