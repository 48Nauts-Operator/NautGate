...
async def test_provider_status_buckets():
    rows = [
        {"provider": "openrouter", "total": 50, "ok": 50, "overloaded": 0,
         "rate_limited": 0, "errors": 0, "retries_absorbed": 0, "last_seen": None},
        {"provider": "anthropic-oauth", "total": 100, "ok": 73, "overloaded": 27,
         "rate_limited": 0, "errors": 0, "retries_absorbed": 5, "last_seen": None},
        {"provider": "down-one", "total": 10, "ok": 0, "overloaded": 10,
         "rate_limited": 0, "errors": 0, "retries_absorbed": 0, "last_seen": None},
    ]
...
def _cr(status_code, error=None):
    return CanaryResult(
        canary_name="heartbeat", via="openrouter", target_provider="openrouter",
        target_model="m", prompt="p", prompt_bytes=1, prompt_tokens=1,
        completion_tokens=1, response_text="OK", response_bytes=2, duration_ms=40,
        first_byte_ms=20, status_code=status_code, cost_usd=0.0, error=error,
    )


async def test_heartbeat_classification(monkeypatch):
    monkeypatch.setattr(provider_heartbeat, "_select_transports",
                        lambda p, m, prefer_oauth: [TargetTransport(via="openrouter", base_url="x", api_key_env="K")])

    async def ok(*a, **k): return _cr(200)
    async def overloaded(*a, **k): return _cr(529)
    async def errored(*a, **k): return _cr(0, error="boom")
    ...
    assert (await provider_heartbeat.ping_once(None, "openrouter", "m", None))["status"] == "degraded"
